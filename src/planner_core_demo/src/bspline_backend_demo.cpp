...
private:
  ...
  void initParams()
  {
    ...
    start_pos_ = Vec3{-5.0, -4.0, 1.0}; start_vel_ = Vec3{0.0, 0.0, 0.0}; goal_pos_ = Vec3{5.0, 4.0, 1.0};
    nv_ = static_cast<int>(2.0 * max_vel_ / vel_resolution_) + 1;

    bspline_samples_per_segment_ = 12;
    knot_dt_ = dt_;                 // B-spline knot span (matches primitive spacing)

    // ESDF + optimisation settings.
    esdf_resolution_ = 0.10;
    opt_iterations_ = 500;
    opt_step_ = 0.08;
    max_move_ = 0.03;               // cap each control point's per-iteration move (stability)
    lambda_smooth_ = 1.0;
    lambda_collision_ = 2.5;
    lambda_feasible_ = 0.12;        // velocity/acceleration penalty (kept small: it is stiff)
    d_safe_ = 0.55;                 // desired clearance to the inflated obstacle
  }
