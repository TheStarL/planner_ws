...
private:
  ...
  void initParams()
  {
    x_min_ = -6.0; x_max_ = 6.0;
    y_min_ = -5.0; y_max_ = 5.0;
    z_min_ = 0.25; z_max_ = 3.5;
    ...
    // Shared optimisation settings (identical for both back-ends so the only
    // difference is HOW the collision gradient is obtained).
    opt_iterations_ = 300;
    opt_step_ = 0.06;
    lambda_smooth_ = 1.0;
    lambda_collision_ = 2.2;
    // Desired clearance kept at the control points.  Because a cubic B-spline only
    // stays inside the convex hull of its control points, the curve between them
    // can come a little closer to an obstacle than the control points themselves,
    // so d_safe is set comfortably above the collision-checking safety margin.
    d_safe_ = 0.6;
    bspline_samples_per_segment_ = 12;

    // ESDF grid resolution (finer grid => more accurate but more expensive to build).
