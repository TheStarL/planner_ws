// ego_planner.cpp
//
// EGO (ESDF-free) trajectory optimisation planner node for the Ackermann car.
// Pipeline: Kino A* front-end → cubic B-spline → analytic collision gradient optimisation.
// Unlike the B-spline planner, no ESDF is built; collision gradients are computed
// directly from the grid map by finding the nearest occupied cell.
//
...
class EgoPlannerNode : public rclcpp::Node
{
public:
  EgoPlannerNode() : Node("ego_planner")
  {
...
private:
  // ---- Kino A* front-end (same as other planners) ----
  ...
  // ---- EGO: analytic collision gradient (no ESDF) ----
  // Finds nearest occupied grid cell and computes direction to it.
  void egoGrad(double x, double y, double & gx, double & gy) const {
    const auto & info = map_loader_.getInfo();
    int cx, cy;
    if (!map_loader_.worldToGrid(x, y, cx, cy)) { gx = gy = 0; return; }

    // Search nearby cells for the nearest occupied one
    int mr = static_cast<int>(d_safe_ / info.resolution) + 1;
    double best_dist = 1e9;
    double best_wx = 0, best_wy = 0;

    for (int dx = -mr; dx <= mr; ++dx) {
      for (int dy = -mr; dy <= mr; ++dy) {
        int gx_idx = cx + dx, gy_idx = cy + dy;
        if (gx_idx < 0 || gx_idx >= info.width || gy_idx < 0 || gy_idx >= info.height) continue;
        if (grid_data_[gy_idx * info.width + gx_idx] <= 50) continue;  // not occupied
        double wx, wy; map_loader_.gridToWorld(gx_idx, gy_idx, wx, wy);
        double d = std::hypot(wx - x, wy - y);
        if (d < best_dist) { best_dist = d; best_wx = wx; best_wy = wy; }
      }
    }

    if (best_dist >= d_safe_) { gx = gy = 0; return; }

    // Direction from obstacle to point
    double vx = x - best_wx, vy = y - best_wy;
    double n = std::hypot(vx, vy);
    if (n < 1e-6) { vx = 1; vy = 0; n = 1; }
    vx /= n; vy /= n;

    double coef = -2.0 * lambda_c_ * (d_safe_ - best_dist);
  ...
  void runEgoOptimization() {
    ...
    for (int iter = 0; iter < opt_iters_; ++iter) {
      ...
      // Collision (EGO: analytic gradient, no ESDF)
      for (size_t i = 0; i < q.size(); ++i) {
        double gx, gy; egoGrad(q[i].x, q[i].y, gx, gy);
    ...
    opt_path_ = sample(q);
    RCLCPP_INFO(this->get_logger(), "EGO opt: %.1f ms, iters: %d, path pts: %zu", ms, opt_iters_, opt_path_.size());
  ...
  int bspline_seg_, opt_iters_; double opt_step_, lambda_s_, lambda_c_, lambda_k_, d_safe_;
...
int main(int argc, char ** argv) {
  ...
  rclcpp::spin(std::make_shared<EgoPlannerNode>());
  rclcpp::shutdown();
  return 0;
}